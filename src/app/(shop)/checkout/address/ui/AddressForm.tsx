...
import type { Address, Country } from '@/src/interfaces';
...
export const AddressForm = ({ countries, userStoredAddress = {} }: Props ) => {
  ...
  const { handleSubmit, register, formState: { isValid }, reset } = useForm<FormInputs>({
    defaultValues:{
      ...(userStoredAddress as any ), // leer desde la base de datos
      rememberAddress: false,
    }
  });

   const { data: session } = useSession({   //data la renombramos como session
    required: true, // si no esta autenticado  manda al login
   })
  ...
  const address = useAdressStore( state => state.address );
    ...
    useEffect(() => {
      ...
      const onSubmit =  async ( data: FormInputs ) => {
        ...
        setAddress(restAddress);  //En la base de datos
